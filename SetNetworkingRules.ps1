...
Set-AzContext -SubscriptionId eac4bec0-ba01-4eb2-803c-2a6c34869126
...
Get-AzNetworkSecurityGroup -Name $Ngs -ResourceGroupName (Get-AzVM -Name $AvaliableVMS[$ChosenVM].Name).ResourceGroupName | Add-AzNetworkSecurityRuleConfig -Name "VPN_Leuven" -Description "Teamcenter and Various Ports that will be opened for Leuven VPN" -Access "Allow" -Protocol "Tcp" -Direction "Inbound" -Priority 101 -SourceAddressPrefix "1.2.3.4/24" -SourcePortRange "*" -DestinationAddressPrefix "*" -DestinationPortRange (80,443,3389,4544,7001,3000) | Set-AzNetworkSecurityGroup
